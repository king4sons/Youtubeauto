...
class PublishRequest(BaseModel):
    project_id: int
    platform: str
    content_type: str = "video"  # video or short
    title: str
    description: str
...
def generate_title_description(
    ...
    return {
        ...
        "descriptions": [
            f"Experience the full cinematic story of {project.title}. This {project.genre or 'cinematic'} masterpiece will take you on a journey you won't forget. Subscribe for more epic content!\n\n#cinema #{(project.genre or 'story').replace(' ', '')} #film #storytelling",
            f"\U0001f3ac {project.title} - A Reckoning Studio Production\n\nImmerse yourself in this gripping {project.genre or 'cinematic'} experience. Full story, professional production, unforgettable moments.\n\n\U0001f4cc Don't miss upcoming episodes!\n#ReckoningStudio #{project.genre or 'Cinema'}",
        ],
        "tags": [
            project.title.lower().replace(" ", ""),
            project.genre or "cinema",
            "reckoning studio",
            "cinematic story",
            "full movie",
            "short film",

...
def generate_video(request: VideoGenerateRequest, background_tasks: BackgroundTasks, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    video = Video(
        ...
        progress=0,
        generation_params={"style": request.style, "script_id": request.script_id, "storyboard_id": request.storyboard_id}
    )
    db.add(video)
    db.commit()
    db.refresh(video)
...
def generate_shorts(video_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    video = db.query(Video).filter(Video.id == video_id).first()
    if not video:
        raise HTTPException(status_code=404, detail="Video not found")
